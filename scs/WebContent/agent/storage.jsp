...
<table bgcolor="lavender" border="0" style="height: 50px；width:60%">
  ...
  <form action="storageaction!rentStorage?userid=<%=user.getUserid()%>&roleid=<%=roleid %>&currentime=<%=currentime %>"  method="post" >
  ...
  <form action="storageaction!buyStorage?userid=<%=user.getUserid()%>&roleid=<%=roleid %>&currentime=<%=currentime %>"  method="post" >
   <tr style="height: 35px">
    <th>
       买仓库的类型    
    </th>
