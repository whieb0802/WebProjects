...
function bill(){
	window.location.href="inventoryaction!rbillconfirm?userid=<%=bill.getBto()%>&billtime=<%=bill.getCurrentime().getCurrentime()%>&up=<%=bill.getBfrom()%>&current=<%=currentime%>&roleid=<%=roleid %>";
}
