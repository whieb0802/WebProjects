...
public class UserInterestTypeHandle implements AttributeHandle {

	public static Map<Long, Item> items = new HashMap<Long, Item>();
	private ItemDao itemDao;
	private InterestDao interestDao;
	public void enterFeedback(IRating r) {
		...
		if (interests != null && interests.size() > 0) {
			for (MyInterest interest : interests) {
				...
				if(interest.getSubjectType() ==1) {
					if (interest.getSubject().equals(item.getType())) {
		...
		if (interests == null || interests.size() == 0 || have == false) {
			...
			interest.setSubjectType(1);
			interest.setUser(userId);
			interest.setWeight(newWeight);
			interest.setSubject(item.getType());
			interestDao.saveInterest(interest);
		}
	}
